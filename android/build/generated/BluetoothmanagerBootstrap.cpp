...
#define TAG "de.appwerft.bluetoothmanager"

using namespace v8;

static Persistent<Object> bindingCache;

static void Bluetoothmanager_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Bluetoothmanager.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::BluetoothmanagerBindings::lookupGeneratedInit(
...
static void Bluetoothmanager_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Bluetoothmanager_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Bluetoothmanager_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::BluetoothmanagerBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry BluetoothmanagerBinding = {
	"de.appwerft.bluetoothmanager",
	Bluetoothmanager_init,
	Bluetoothmanager_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_de_appwerft_bluetoothmanager_BluetoothmanagerBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.appwerft.bluetoothmanager", &BluetoothmanagerBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::BluetoothmanagerBindings::lookupGeneratedInit));
}
